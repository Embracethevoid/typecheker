...
int main(int argc, char **argv) {
  ...
  while (token != EOF) {
	...
	if (name == "MISS")
	{
	std::cout<<yylineno<<' '<<"STRING_LIT"<<" \""<<yytext<<"\""<<std::endl;
	std::cerr<<yylineno<<':'<<"Unclosed string?  Encountered newline in quoted string. (at '"<<yytext<<"'"<<std::endl  ;
...
else if (name == "ILLEGAL")
{
	...
	std::cout<<yylineno<<' '<<"STRING_LIT"<<" \""<<yytext<<" \""<<std::endl;
	std::cerr<<yylineno<<':'<<"Illegal escape code; only \\\\, \\0, \\t, \\n, \\r, \\n are 	permitted (at '"<<yytext<<"'"<<std::endl ;
